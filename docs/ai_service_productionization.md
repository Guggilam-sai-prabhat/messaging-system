...
Failure mode: **implemented** as a local in-process fallback rather than a pure fail-open. `RateLimitService` catches the Redis error and hands the same key off to an in-process `LocalWindowCounter` (a `deque` of monotonic timestamps per key, same fixed-window semantics), enforcing the *same* `AI_RATE_LIMIT_USER_MAX`/`CHANNEL_MAX` thresholds locally. With today's single-consumer-process deployment this is equivalent protection to the Redis path. If/when multiple consumer processes are introduced (§2), each process would enforce the limit independently against its own local counter during a Redis outage — so the effective combined limit becomes `N processes × configured limit` for that window, a known and accepted degradation rather than a silent gap. This still beats both extremes: failing fully open (zero protection) and failing fully closed (blocking the whole AI feature on a Redis blip).

### Concurrent-call cap toward NIM (the second, distinct limit)

Per-user/channel limits don't bound total load if you later run multiple consumer instances. Instead of a semaphore gate, run a fixed pool of `NVIDIA_MAX_CONCURRENT_CALLS` (default 4–8, tunable) worker tasks in `ai_service/rag/generator.py`, each pulling requests off a bounded `asyncio.Queue` (maxsize = pool size × small multiplier, e.g. 2x, to allow a little buffering without unbounded backlog). Callers `await queue.put((request, future))` and `await future` for the result instead of calling `NvidiaChatClient.chat_completion` directly; each worker loop does `request, future = await queue.get()` → call NIM → `future.set_result(...)` → loop.

This is equivalent in effect to the semaphore (still caps concurrent NIM calls at N) but gives an explicit, boundedly-sized queue instead of N tasks racing on a semaphore — which makes queue depth directly observable (`queue.qsize()` as a gauge, see §3) and makes backpressure explicit: if the queue is full, `put` blocks (or times out, if you use `put_nowait` + except `QueueFull` to fail fast instead of blocking indefinitely). This is not a "rate limit" in the abuse-prevention sense — it's a concurrency cap protecting NIM and your own cost/latency, and it composes with whatever consumer-count scaling you do (see §2).
...
Within a single process, add intra-process concurrency instead of scaling processes first (cheaper, no new infra): replace the fully-serial `poll → handle → commit` with N worker tasks (`WORKER_CONCURRENCY`, default 4) consuming from a bounded `asyncio.Queue`, rather than a semaphore wrapping per-message tasks. The poll loop becomes a single producer — `await queue.put(message)` per polled message (blocking/backpressuring the poll loop itself when the queue is full, which is a feature: it naturally slows consumption when workers can't keep up, instead of spawning unbounded in-flight tasks under a semaphore) — and each worker loop does `message = await queue.get()` → `handle(message)` → done, forever. This is functionally similar to the semaphore approach (both cap in-flight work at N) but makes the backlog an inspectable object (`queue.qsize()`) rather than an implicit count of tasks waiting on the semaphore, and cleanly separates "fetching from Kafka" from "processing," which is useful if those two ever need independent tuning.

Either way, offsets must be committed **per-partition, in offset order**, not per-message-completion order (async completion can reorder — a queue-fed worker pool reorders completions just as much as a semaphore-gated one does). This requires tracking the lowest uncommitted offset per partition — a small queue/watermark structure (distinct from the work-distribution queue above), not a large rewrite, but worth calling out as the trickiest correctness piece of this whole plan.
...
| **Redis unavailable** | Rate limiting (§1) falls back to an in-process `LocalWindowCounter` enforcing the same thresholds — real protection under a single consumer process, degraded (per-process independent limits) under multiple. Reply dedup (§2) still fails fully open (no equivalent local fallback implemented) — a duplicate reply is possible during a Redis outage. | Consider adding a similar local fallback for reply dedup if duplicate replies during Redis outages become a real problem in practice; today it's accepted as the lesser risk (a dropped/duplicate reply vs. the whole AI feature going dark). |
